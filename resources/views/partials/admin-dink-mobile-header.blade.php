        ...
        <details class="relative">
            ...
            <div class="absolute right-0 mt-2 w-56 bg-forest rounded-lg shadow-lg py-2 z-20">
                @foreach ($navItems as $item)
                    <a href="{{ route($item['route']) }}" class="block px-4 py-2 text-sm {{ request()->routeIs(str_replace('.index', '.*', $item['route'])) || request()->routeIs($item['route']) ? 'text-white font-semibold' : 'text-white/70' }} hover:text-white hover:bg-white/5">
                        {{ $item['label'] }}
                    </a>
                @endforeach
                <div class="border-t border-white/10 my-2"></div>
                <div class="px-4 py-1 text-xs text-white/60">{{ auth()->user()->name }} &middot; {{ auth()->user()->role->label() }}</div>
                <a href="{{ route('profile.edit') }}" class="block px-4 text-sm text-white/70 hover:text-white py-1">My Account</a>
                <form method="POST" action="{{ route('logout') }}" class="px-4 pt-1">
                    @csrf
                    <button type="submit" class="text-sm text-white/70 hover:text-white py-1">Logout</button>
                </form>
            </div>
        </details>
    </div>
