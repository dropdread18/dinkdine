...
@if ($payment)
    <x-card class="max-w-sm space-y-3 text-sm mt-4">
        ...
        @php
            // Payments (viewing the list, and every action here) reverted
            // to admin-only - owner feedback after trying the earlier
            // staff-can-mark-paid change. See routes/web.php for the
            // matching route split.
            $canMarkPaid = auth()->user()->isAdmin() && in_array($payment->status, [\App\Enums\PaymentStatus::Unpaid, \App\Enums\PaymentStatus::Pending, \App\Enums\PaymentStatus::Failed], true);
