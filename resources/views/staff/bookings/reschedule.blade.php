...
@section('content')
    ...
    <div class="flex items-center justify-between mb-2">
        ...
        <div class="flex items-center gap-3 text-sm">
            @if ($prevDate >= $minDate)
                <a href="{{ route('manage.bookings.reschedule', ['booking' => $booking, 'date' => $prevDate]) }}" class="text-gray-600 hover:text-gray-900">&lt;</a>
            @else
            ...
            @if ($nextDate <= $maxDate)
                <a href="{{ route('manage.bookings.reschedule', ['booking' => $booking, 'date' => $nextDate]) }}" class="text-gray-600 hover:text-gray-900">&gt;</a>
            @else
                <span class="text-gray-300">&gt;</span>
            @endif
        </div>
